"""Poker44 SN126 miner — depth-matched bot detector (canary line).

Serves P(focus player is a bot) per chunk using the p44depth detector: the
recent-regime gradient-boosted fleet fed features from training-depth (~34-hand)
bagged subsamples of each live chunk, combined with two public reference models by a
z-weighted mean. Independent implementation kept deliberately separate from the
...
from p44depth.detector import Detector
...
def _declared_implementation(root: Path):
    ...
    return [
        root / "p44depth" / "detector.py",
        root / "p44depth" / "aggregate.py",
        root / "neurons" / "miner_depth.py",
    ]
...
class Miner(BaseMinerNeuron):
    def __init__(self, config=None):
        ...
        bt.logging.info("🃏 p44depth depth-matched detector ready")

        self.model_manifest = build_local_model_manifest(
            repo_root=_ROOT,
            implementation_files=_declared_implementation(_ROOT),
            defaults={
                "model_name": os.getenv("POKER44_MODEL_NAME", "poker44-depthmatch-detector"),
                "model_version": os.getenv("POKER44_MODEL_VERSION", "1"),
                "framework": "gradient-boosting-ensemble",
                "license": "MIT",
                "repo_url": os.getenv("POKER44_MODEL_REPO_URL", ""),
                "repo_commit": os.getenv("POKER44_MODEL_REPO_COMMIT", _head_commit(_ROOT)),
                "notes": (
                    "Depth-matched gradient-boosted fleet (features from ~34-hand bagged "
                    "subsamples matching training chunk depth) plus public reference models, "
                    "combined by a z-weighted mean."
                ),
                "open_source": True,
...
if __name__ == "__main__":
    with Miner() as node:
        bt.logging.info("Poker44 depth-matched miner running.")
        while True:
            try:
                mg = node.metagraph

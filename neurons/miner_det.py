"""Poker44 SN126 miner — rank-consensus bot detector (challenger line).

Serves P(focus player is a bot) per chunk using the p44det consensus detector:
a recent-regime gradient-boosted fleet plus public reference models combined by
robust rank-consensus aggregation. This is an independent implementation kept
deliberately separate from the baseline gradient-boosted detector.
"""
import os
import sys
import time
...
from p44det.detector import Detector
...
def _declared_implementation(root: Path):
    ...
    return [
        root / "p44det" / "detector.py",
        root / "p44det" / "aggregate.py",
        root / "neurons" / "miner_det.py",
    ]
...
class Miner(BaseMinerNeuron):
    def __init__(self, config=None):
        ...
        bt.logging.info("🃏 p44det consensus detector ready")

        self.model_manifest = build_local_model_manifest(
            repo_root=_ROOT,
            implementation_files=_declared_implementation(_ROOT),
            defaults={
                "model_name": os.getenv("POKER44_MODEL_NAME", "poker44-consensus-detector"),
                ...
                "notes": (
                    "Rank-consensus ensemble of a recent-regime gradient-boosted fleet "
                    "with robust component aggregation."
                ),
                "open_source": True,
...
if __name__ == "__main__":
    with Miner() as node:
        bt.logging.info("Poker44 consensus miner running.")
        while True:
            try:
                mg = node.metagraph

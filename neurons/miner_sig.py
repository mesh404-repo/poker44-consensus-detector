"""Poker44 SN126 miner — drift-robust signature-uniqueness bot detector.

Serves P(focus player is a bot) per chunk using the p44sig detector: a gradient-boosted model over
only the feature core whose meaning survives the benchmark->live shift (signature-uniqueness,
entropy and share ratios), with the decision boundary recentred per window. Independent
implementation kept separate from the baseline, depth-matched and transformer detectors.
"""
import os
import sys
import time
...
from p44sig.detector import Detector
...
def _declared_implementation(root: Path):
    ...
    return [
        root / "p44sig" / "detector.py",
        root / "p44xf" / "xfnet.py",
        root / "neurons" / "miner_xf.py",
    ]
...
class Miner(BaseMinerNeuron):
    def __init__(self, config=None):
        ...
        bt.logging.info("🃏 p44sig drift-robust signature detector ready")

        self.model_manifest = build_local_model_manifest(
            repo_root=_ROOT,
            implementation_files=_declared_implementation(_ROOT),
            defaults={
                "model_name": os.getenv("POKER44_MODEL_NAME", "poker44-signature-robust"),
                "model_version": os.getenv("POKER44_MODEL_VERSION", "6"),
                ...
                "notes": (
                    "Gradient-boosted model over the drift-robust feature core "
                    "(signature-uniqueness, entropy and share ratios), with a per-window "
                    "recentred decision boundary."
                ),
                "open_source": True,
...
if __name__ == "__main__":
    with Miner() as node:
        bt.logging.info("Poker44 signature-robust miner running.")
        while True:
            try:
                mg = node.metagraph
